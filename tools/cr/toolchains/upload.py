...
DEFAULT_KMS_ALGORITHM = 'RSASSA_PSS_SHA_256'
...
# Stream files through the hasher in 1 MiB chunks so a large file never lands on
# the heap in one piece.
...
class ArtifactSignature:
    ...
    # Base64-encoded signature bytes, as returned by `aws kms sign`.
    signature: str
...
def sha256_file(path: Path) -> str:
    """Return the hex SHA-256 of *path*, streamed in chunks."""
...
def kms_verify(sha256_hex: str,
    ...
    _check_output('aws', 'kms', 'verify', '--region', region, '--key-id',
                  signature.key_id, '--signing-algorithm', signature.algorithm,
...
class S3Uploader:
    ...
    Binds the destination bucket and the AWS region + KMS key/algorithm shared
    across uploads. Each call `upload()` supplies the file and its key. Uses
    the `aws` CLI with ambient credentials (see the module docstring).
    """
    ...
    def __init__(self,
                 ...
                 kms_algorithm: str = DEFAULT_KMS_ALGORITHM,
        ...
        Args:
            ...
            kms_algorithm: KMS signing algorithm.
...
def main() -> int:
    ...
    parser.add_argument('--kms-algorithm', default=DEFAULT_KMS_ALGORITHM)
    ...
    uploader = S3Uploader(bucket=args.bucket,
                          ...
                          kms_algorithm=args.kms_algorithm,

...
content::BrowserContext* TabActivatorFactory::GetBrowserContextToUse(
  ...
  // Only regular profiles get their own activator. Off-the-record and other
  // ephemeral profiles are intentionally excluded: SnapshotOpenTabs (in
  // TabSemanticSearchTool) filters to `TYPE_NORMAL` browsers owned by the
  // regular profile, so an incognito activator would have nothing to act on,
  // and TabTrackerService only ever holds one activator at a time.
